...
__all__ = [
    'PermManager', 'Manager', 'Model', 'PROTECT', 'CASCADE', 'SET_NULL', 'SET', 'SET_DEFAULT', 'DO_NOTHING',
    'F', 'Value', 'Func', 'AutoField', 'BigAutoField', 'BigIntegerField', 'BinaryField', 'BooleanField',
    'CharField', 'DateField', 'DateTimeField', 'DecimalField', 'DurationField', 'EmailField', 'Field',
    'FilePathField', 'FloatField', 'ForeignKey', 'GenericIPAddressField', 'IntegerField', 'ManyToManyField',
...
from .fields import *
from django.db.models import PROTECT, CASCADE, SET_NULL, SET, SET_DEFAULT, DO_NOTHING, Manager, F, Value, Func
...
class Model(models.Model):
    ...
    def readable_fields_name(cls, user):
        return [field.name for field in cls._meta.fields if
                (not hasattr(field, 'has_read_perm') or field.has_read_perm(user))]

    @classmethod
    def writeable_fields_name(cls, user):
        return [field.name for field in cls._meta.fields if
                (not hasattr(field, 'has_write_perm') or field.has_write_perm(user))]
